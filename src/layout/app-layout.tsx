...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['restaurant-owner', 'chef'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PizzaRestaurantInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/pizza-restaurants` : null),
    () => getPizzaRestaurants({ filter: { tenant_id: session?.user?.tenantId } }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
...

    {
      ...
      path: isTenantUser ? `/pizza-restaurants/${data?.[0]?.id}` : '/pizza-restaurants',
